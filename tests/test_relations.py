...
class ModelDatasetRelationTests(unittest.TestCase):
    ...
    def test_dataset_lineage_index_and_backlinks_are_symmetric(self) -> None:
        self.assertEqual(len(self.payload["dataset_relations"]), 54)
        relation_pairs = {
        ...
        self.assertIn(
            ("id-v2v-poly-haven-hdri-sample", "id-v2v-face-relighting-pairs"),
            relation_pairs,
        )
        ...
        self.assertIn(("moviebench", "insave-80k"), relation_pairs)
        self.assertIn(("condensed-movies", "insave-80k"), relation_pairs)
        self.assertIn(("sf20k", "insave-80k"), relation_pairs)
        self.assertIn(("vggsound", "insave-80k"), relation_pairs)
        self.assertIn(("vggsound", "aved-bench"), relation_pairs)
    ...
    def test_dataset_lineage_preserves_relationship_evidence(self) -> None:
        ...
        self.assertIn("parts 001 through 030", relation["contribution"])
    ...
    def test_model_monitoring_is_joined_by_canonical_model_id(self) -> None:
        ...
        self.assertEqual(len(ranked_model_ids), 51)
        self.assertEqual(
            {
                model_id
